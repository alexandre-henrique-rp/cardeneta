...
const formatInputCurrency = (value: string, previousValue: string = "") => {
...
export function AtmForm({ mode, data }: AtmFormProps) {
  ...
  const getCurrentLocation = (): Promise<{ latitude: number | null; longitude: number | null; accuracy: number | null }> => {
  ...
  useEffect(() => {
    if (mode === "edit" && data) {
      ...
      const paymentDate = data.paymentDueDate ? new Date(data.paymentDueDate) : null;
      ...
      form.reset({
        ...
        type: data.type || "",
        paymentDueDate: paymentDate ? new CalendarDate(
  ...
  const loadProofUrl = async (proofId: string) => {
    try {
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    ...
    try {
      ...
      const payload = {
        ...
        paymentDueDate: values.paymentDueDate ? values.paymentDueDate.toDate(getLocalTimeZone()) : undefined,
        gps: gpsLocation,
              ...
              <FormItem>
                ...
                <FormControl>
                  <Input
                    ...
                    value={currencyValue}
                    onChange={(e) => {
                      const newValue = formatInputCurrency(e.target.value, currencyValue);
              ...
              {!uploadedFile && !existingProofUrl ? (
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between p-3 bg-muted rounded-md">
                    <div className="flex items-center space-x-3">
                      ...
                      <div className="text-left">
                        ...
                        <p className="text-xs text-muted-foreground">
                          {getFileTypeFromUrl(existingProofUrl) === 'pdf' ? 'PDF' : 'Imagem'}
                        </p>
                      </div>
                    </div>
                    <div className="flex gap-2">
                      ...
                      {data?.proofId && (
                        <Button
                          ...
                          variant="outline"
                          size="sm"
                          onClick={() => window.open(`${import.meta.env.VITE_API_URL}/comprovante/download/${data.proofId}`, '_blank')}
                        >
                  ...
                  {data?.proofId && (
                    <div className="text-center">
                      <p className="text-sm text-muted-foreground">
                        Para substituir o comprovante, primeiro exclua o atual clicando no botão ✕ acima
                      </p>
                    </div>
                  )}
                </div>
