...
export function useWebAuthn() {
  ...
  const register = async () => {
    ...
    try {
      ...
      const registrationResponse = await startRegistration({ optionsJSON: challengeResponse });
   ...
   */
  const login = async (): Promise<{ token: string; user: User; expiresAt: number } | null> => {
    setIsLoading(true);
    setError(null);
    try {
      const challengeResponse = await webAuthnApi.getLoginChallenge();
      const authResponse = await startAuthentication({ optionsJSON: challengeResponse });
      const { token, user, expiresAt } = await webAuthnApi.verifyLogin(authResponse);
  ...
  return { 
    ...
    error, 
    isPwaMode,
    hasBiometricRegistration: () => !!localStorage.getItem(BIOMETRIC_REGISTERED_KEY),
  };
}
